...
class VocabMaster {
    ...
    openInGoogleTranslate(text, src = 'auto', tl = 'en') {
        try {
            ...
            if (/Android/i.test(ua)) {
                // Use an ACTION_SEND intent with the text as the standard EXTRA_TEXT so
                // the Translate app receives the text. Include a browser_fallback_url.
                // Example intent URI format:
                // intent:#Intent;action=android.intent.action.SEND;type=text/plain;S.android.intent.extra.TEXT=...;package=com.google.android.apps.translate;S.browser_fallback_url=...;end
                const safeText = encodeURIComponent(text);
                const intentUrl = `intent:#Intent;action=android.intent.action.SEND;type=text/plain;S.android.intent.extra.TEXT=${safeText};package=com.google.android.apps.translate;S.browser_fallback_url=${encWeb};end`;
                // Navigate to the intent URI (user gesture present)
    ...
    async loadVersion() {
        try {
            // Build a resilient list of candidate URLs for the VERSION file.
            // Use absolute root, common src path, and document-relative URL so
            // the file is discoverable when the app is served from different
            // base paths or loaded inside WebViews / Android intents.
            const origin = (window.location && window.location.origin) ? window.location.origin : '';
            const docBase = (window.location && window.location.href) ? window.location.href.replace(/[^/]*$/, '') : '';
            const candidates = [
                `${origin}/VERSION`,           // absolute root
                `${origin}/src/VERSION`,       // common src path
                new URL('VERSION', window.location.href).href,   // document-relative
                new URL('../VERSION', window.location.href).href, // one level up
                new URL('./VERSION', window.location.href).href
            ];
            let v = null;
